...
let deleteInvoice = () => {
    loading.value = true;
    isDeleteModal.value = false;
    router.delete(route('invoices.destroy', deleteId.value), {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
        ...
        <div class="py-10 max-w-8xl mx-auto">
            <div id="content" class="">
                ...
                <div class="shadow-md sm:rounded-lg mt-5">
                    ...
                    <!--                        </div>-->
                    <!--                        <div class="mt-2">-->
                    <!--                            <label for="group_id" class="text-lg font-semibold px-2">Customer</label>-->
                    ...
                    <div class="mt-10 w-full">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tr class="space-x-2 space-y-2 border  divide-x-2 w-full">
                                ...
                                <th scope="col" class="p-2 w-4/12">Date</th>
                                <th scope="col" class="p-2 w-4/12">Customer</th>
                            ...
                            <tr class="border divide-x-2 w-full text-sm"
                                v-for="(invoice, key) in invoices.data">
                                <td class="text-center text-2xl font-semibold">{{ key + 1 }}</td>
                                <td class="text-left px-2 ">{{ dateFormat(invoice.date) }}</td>
                                <td class="text-left px-2 ">{{ invoice?.customer?.name }}</td>
                                <td class="px-2 text-end">{{ invoice.subtotal }}</td>
                                <td class="px-2 text-end">{{ invoice.discount_amount }}</td>
                                <td class="px-2 text-end">{{ invoice.total }}</td>
                                <td class="px-2 text-end">{{ invoice.payment }}</td>
                                <td class="px-2 text-end">{{ invoice.due }}</td>

                                <td class="text-center">
                                    <Link class="rounded-md" :href="route('invoices.edit',invoice.id)">
                                        <ion-icon name="create-outline" class="text-2xl text-red-600"></ion-icon>
                                    </Link>
                                    <button class=" rounded-md" @click="openDeleteModal(invoice.id)">
